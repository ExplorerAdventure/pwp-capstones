...
def prepare_text(text):
    for letter in text:
        prepared_text = text.replace('!','').replace('?','').replace("\"","").lower().replace(",","").replace(".","").split()
...
def frequency_comparison(table1, table2):
    ...
    for key in table2:
        if not key in table1:
            appearances += table2[key]
    freq_score = float(mutual_appearances) / appearances
    return freq_score
...
tt1 = build_frequency_table(prepare_text(murder_note))
tt2 = build_frequency_table(prepare_text(lily_trebuchet_intro))
print(frequency_comparison(tt1,tt2))
